...
class Transaction(models.Model):
    ...
    def __str__(self):
        return f"{self.name} ({self.type}) - {self.amount} by {self.user.name}"
...
class Refund(models.Model):
    ...
    transaction = models.OneToOneField(Transaction, on_delete=models.CASCADE, related_name="refund_transaction")
    ...
    def __str__(self):
        return f"Refund {self.mode} - {self.amount} for {self.transaction.name}"
    
class PayLater(models.Model):
    paylaterid = models.UUIDField(default=uuid.uuid4, primary_key=True, editable=False, unique=True)
    transaction = models.OneToOneField(Transaction, on_delete=models.CASCADE, related_name="paylater_transaction")
    title = models.CharField(max_length=255)
    amount = models.DecimalField(max_digits=10, decimal_places=2)
    ...
    def __str__(self):
        return f"PayLater {self.amount} due on {self.due_date} for {self.transaction.name}"
